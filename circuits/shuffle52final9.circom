...
template ShuffleReencryptDeal9Commit(N) {
    // Public inputs
    signal input pk[2];
    signal input inputDeckCommit;
    signal input outputDeckCommit;

    // Public dealt cards: outputDeck[0] and outputDeck[1]
    signal input dealt_c1[9][2];
    signal input dealt_c2[9][2];
    ...
    // 8) Bind dealt cards to output deck positions - player private cards and community cards
    var dealtPositions[9] = [0, 1, 2, 3, 5, 6, 7, 9, 11];

    for (var i = 0; i < 9; i++) {
        dealt_c1[i][0] === cout_c1[dealtPositions[i]][0];
        dealt_c1[i][1] === cout_c1[dealtPositions[i]][1];

        dealt_c2[i][0] === cout_c2[dealtPositions[i]][0];
        dealt_c2[i][1] === cout_c2[dealtPositions[i]][1];
    }
}
...
} = ShuffleReencryptDeal9Commit(52);